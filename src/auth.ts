...
export function getCredentialStatus(flags: CliFlags = {}): Record<string, unknown> {
  const envSession = process.env[String(flags.sessionEnv || 'YUQUE_SESSION')]
  const envCtoken = process.env[String(flags.ctokenEnv || 'YUQUE_CTOKEN')]
  if (envSession && envCtoken) {
    return {
...
export async function recordCredentialValidation(ok: boolean, error?: unknown): Promise<void> {
  const saved = readSavedCredentials()
  if (!saved?.session || !saved?.ctoken) return
...
function ageHours(value: string | undefined): number | null {
...
function getConfiguredHomeUrl(flags: CliFlags): string | undefined {
...
function normalizeYuqueHomeUrl(value: string): string {
  ...
  try {
    ...
    if (!login) throw new Error('Yuque home URL must include your login or organization path.')
    url.hash = ''
    url.search = ''
    url.pathname = `/${login}/`
    return url.toString()
  } catch (error) {
    if (error instanceof Error && error.message.startsWith('Yuque home URL')) throw error
