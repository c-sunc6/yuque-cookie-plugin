...
import type { SerializeHtmlToLakeOptions, SerializeHtmlToLakeResult } from './types.ts'

const VENDOR_DIR = path.resolve('vendor/lake-editor')

export async function serializeHtmlToLake({ html, out, keepTemp = false }: SerializeHtmlToLakeOptions): Promise<SerializeHtmlToLakeResult> {
  ...
  try {
    ...
    const result = await page.evaluate(async (input) => {
      return (window as any).__yuqueBridge.serialize(input)
    }, html) as SerializeHtmlToLakeResult
    if (!result?.lake?.trim()) throw new Error('Editor bridge returned empty Lake content.')
    if (out) {
...
async function hasOfficialEditorAssets(): Promise<boolean> {
  try {
...
function bridgeHtml({ vendorAvailable }: { vendorAvailable: boolean }): string {
