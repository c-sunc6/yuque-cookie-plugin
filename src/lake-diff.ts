import { parseLake, type LakeCard, type LakeHeading, type ParsedLake } from './lake-parser.ts'
import type { YuqueSnapshot } from './types.ts'

export function diffLakeSnapshots(before: YuqueSnapshot, after: YuqueSnapshot): Record<string, unknown> {
...
export function extractLakeHeadings(lake: string): LakeHeading[] {
  const result: LakeHeading[] = []
...
function diffHeadings(before: LakeHeading[], after: LakeHeading[]): Array<Record<string, unknown>> {
...
function summarizeHeading(heading: LakeHeading): Record<string, unknown> {
  return {
    level: heading.level,
    text: heading.text,
    attrs: heading.attrs,
...
function summarizeStructure(parsed: ParsedLake): string[] {
  return [
...
function countByName(items: LakeCard[]): Record<string, number> {
  const counts: Record<string, number> = {}
  for (const item of items) counts[item.name] = (counts[item.name] || 0) + 1
  return counts
}

function diffNamedCounts(before: Record<string, number>, after: Record<string, number>): Array<Record<string, unknown>> {
