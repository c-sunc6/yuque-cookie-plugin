...
export class YuqueCookieClient {
  ...
  async createBook(params: { name: string; slug: string; description?: string; public?: boolean }): Promise<Record<string, unknown>> {
    ...
    const created = await this.requestJson('POST', '/api/books', payload, referer) as Record<string, any>
  ...
  async createMarkdownDoc(bookUrl: string, params: {
    title: string
    body: string
    slug?: string
    attachToToc?: boolean
    ...
    if (!book?.id) throw new Error('URL is not a readable Yuque book page.')
    if (!book.abilities?.create_doc) throw new Error(`Current account cannot create docs in book "${book.name || book.slug || book.id}".`)
    ...
    const created = await this.requestJson('POST', '/api/docs', payload, bookUrl) as Record<string, any>
    const doc = created.data || created
    const toc = Array.isArray(created.toc) ? created.toc : []
    const tocAttached = toc.some((item: Record<string, any>) => Number(item.doc_id || item.id) === Number(doc.id))
  ...
  async uploadAttach(refererUrl: string, filePath: string): Promise<Record<string, unknown>> {
    const appData = await this.fetchAppData(refererUrl)
    const me = appData.me
    if (!me?.id) throw new Error('Failed to resolve current Yuque user id for upload.')
    const absolutePath = path.resolve(filePath)
    const info = await stat(absolutePath)
    if (!info.isFile()) throw new Error(`Upload path is not a file: ${filePath}`)
    ...
    const upload = sanitizeUploadResponse((response as Record<string, any>).data || response)
    ...
    if (!res.ok) throw new Error(`POST ${safeEndpoint} failed: ${res.status} ${text.slice(0, 500)}. Report: ${report}`)
    return {
      ok: true,
      file: {
        name: fileName,
...
function buildDocUrl(host: string, appData: Record<string, any>, book: Record<string, any>, doc: Record<string, any>): string {
  const spaceLogin = appData.space?.login || appData.me?.login
  const bookSlug = book.slug
  const docSlug = doc.slug || doc.id
  if (spaceLogin && bookSlug && docSlug) return `${host}/${spaceLogin}/${bookSlug}/${docSlug}`
  if (bookSlug && docSlug) return `${host}/${bookSlug}/${docSlug}`
...
function sanitizeUploadResponse(upload: Record<string, any>): Record<string, unknown> {
