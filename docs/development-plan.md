...
| 2026-06-24 | Add retry report plans | Added structured `retry` plans to `download-book` and `download-doc` reports so failed docs can be retried via `download-doc` with preserved options | Targeted book/doc download tests and typecheck pass; full suite reached 59 tests | Retry plans cover failed documents, not failed media resources yet | Consider media retry/report grouping after complex real fixtures |
| 2026-06-24 | Add resource warning summaries | Added `warning_summary` with type counts and retryable resource URLs; tests cover attachment warning summaries and media download failures | Targeted resource tests pass; full suite expected to reach 61 tests | Resource retry is still a report aid, not a standalone downloader command | Consider a future `retry-resources` command only if real usage needs it |
| 2026-06-24 | Expand media API compatibility | Added a media info normalization layer for alternate Yuque API fields such as `download_url`, `downloadUrl`, `src`, `play_url`, and nested `file` metadata | Targeted media tests and typecheck pass; full suite expected to reach 63 tests | Still needs real private-doc media fixtures to prove all field variants | Run `real:acceptance` on docs containing audio/video cards |
| 2026-06-24 | Validate extra cookie download path | Added cross-process CLI download coverage for `--cookie-key` / `--cookie-value`, using a mock book that requires `verified_books=ok` | Targeted CLI download tests pass; full suite expected to reach 64 tests | Extra cookies are compatibility helpers, not a Yuque token replacement | Validate against a real verified/private-password book when available |
| 2026-06-24 | Cover article output options | Added structured tests for `hideFooter` and `convertMarkdownVideoLinks`, replacing old snapshot expectations with focused assertions | Targeted article tests and typecheck pass; full suite expected to reach 66 tests | Conversion is intentionally simple and only targets Markdown mp4/mp3 links | Keep richer media cards on the `localizeMedia` path |
| 2026-06-24 | Enhance real doc/media acceptance | Extended `real:acceptance` doc mode to surface `warning_summary`, retry plans, downloader report paths, and per-file existence/preview checks | `npm run real:acceptance --` fails fast without URL; `npm test` passes with 12 files and 66 tests | Real audio/video parity still requires a real document containing those cards | Run against a media-heavy Yuque doc and use `warning_summary.retryable_resources` for follow-up |
| 2026-06-24 | Add serve-book CLI config smoke | Added `serve-book --config-only` so automation can generate and validate VitePress config without starting a long-running server; covered it through a real CLI subprocess test | Targeted CLI/serve-book tests and typecheck pass | Live VitePress process remains covered by injectable server tests, not a long-running CLI test | Keep config-only as the stable automation smoke path |
| 2026-06-24 | Add downloaded resource manifests | Added `resources` manifests to `download-book` and `download-doc` reports, listing localized image/attachment files with type counts and byte sizes | Targeted downloader tests pass | Manifest reports local files only; failed resources remain in `warning_summary` | Use manifest plus warnings during real audio/video acceptance |
| 2026-06-24 | Surface resource manifests in real acceptance | Added `resources`, `warning_summary`, downloader report paths, and resource file checks to `real:acceptance` book/doc reports | `npm run real:acceptance --` fails fast without URL; `npm test` passes with 12 files and 71 tests | Real resource coverage still depends on selected Yuque documents | Run real acceptance against media-heavy docs |
| 2026-06-25 | Harden real auth validation and login UI | Added `auth-status`, credential validation timestamps, relogin hints, and a GSAP-enhanced local login/success page | Pending validation in this iteration | Final release acceptance must use real Yuque documents, not mocks | Run auth-status and real acceptance with current browser cookies |
| 2026-06-25 | Validate native Lake document creation | Confirmed the default `create-doc` flow creates Markdown through Yuque, snapshots generated `body_asl`, then writes back Lake; verified native heading numbering uses `data-lake-index-type="2"` instead of textual numbering | Real snapshots under `reports/real-ai-use-numbered/` show native headings, paragraphs, lists, `lake-table`, and `codeblock` card | Images, attachments, mindmaps, and boards still need real before/after exploration | Use `docs/native-lake-capability.md` as the acceptance matrix for rich native writes |
| 2026-06-25 | Validate native rich text structures | Created a real Yuque document `AI真实测试-原生富文本结构`, snapshotted it, and confirmed ordered lists, nested lists, blockquote, hr, bold, italic, delete, links, tables, codeblock, and heading numbering are Lake-native | Snapshot: `reports/real-ai-use-native-rich/snapshot-native-rich.json`; downloaded copy has zero resource warnings | Upload-backed resources are still not covered by this test | Explore image/attachment upload next through `/api/upload/attach` before inserting cards |
| 2026-06-25 | Validate and implement native image insertion | Added `upload-attach` and `insert-image`; verified local PNG upload through `/api/upload/attach`, official editor image card serialization, dry-run, real `apply-lake`, after snapshot, and downloader resource localization | Real doc `gkaswg4ccuoi4gm3`; snapshot `reports/real-ai-use-native-image/snapshot-after-image.json`; download resources reported 1 image and 0 warnings | `insert-image --dry-run` still uploads the file because the native card requires a real CDN URL | Add attachment insertion using the same upload foundation |
| 2026-06-25 | Validate native PDF attachment insertion | Added `insert-attachment` and fixed file card metadata after real failure: Yuque reader requires `src + name`; `src` must point to `/office/<filekey>?from=<doc-url>`, while `downloadUrl/download_url` preserve the raw attachment URL for local export | Real A99 PDF upload and insertion succeeded; user confirmed the latest inserted attachment opens correctly instead of `about:blank`; snapshot `reports/real-ai-use-native-attachment/snapshot-after-a99-src-attachment.json` | Older test file cards without `src` remain bad examples in the test doc and may show empty links | Use the validated file card pattern for future PDF/attachment workflows |
