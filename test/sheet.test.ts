...
import { genMarkdownTable, parseSheet } from '../src/download-utils.ts'
...
describe('sheet parser migrated from yuque-dl', () => {
  it('parseSheet converts compressed Yuque sheet data to markdown table text', () => {
    const data = parseSheet(sheetData)
    expect(data).toContain('## Sheet1')
    expect(data).toContain('## Sheet2')
    expect(data).toContain('GSSSAPPAP')
    expect(data).toContain('[baidu](https://www.baidu.com)')
  })

  it('genMarkdownTable converts sparse sheet data', () => {
    const data = genMarkdownTable({
      0: { 0: { v: '单元格(0,0)' } },
      2: { 2: { v: '单元格(2,2)' } },
      4: { 4: { v: '单元格(4,4)' } }
    })
    expect(data).toContain('| |A | B | C | D | E|')
    expect(data).toContain('| 1 | 单元格(0,0) |  |  |  | |')
    expect(data).toContain('| 5 |  |  |  |  | 单元格(4,4)|')
  })
})
